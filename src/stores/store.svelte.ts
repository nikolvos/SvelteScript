...
export const folderStructureStore=writable([
	{
...
export const folderStructure = get(folderStructureStore);

export const questions = get(questionsStore);
...
export function addQuestion(questionData:NewQuestionInput) {
	const { folderName, question, answerCode, answerAlgorithm } = questionData;

	// 1. Generate ID based on existing questions
	const currentQuestions = get(questionsStore);
	const sameFolder = currentQuestions.filter((q) => q.id.startsWith(folderName + '/'));

	const newIndex = sameFolder.length + 1;
	const newId = `${folderName}/${newIndex}`;
	...
	// 2. Update questions store
	questionsStore.update((qs) => [...qs, newQuestion]);

	// 3. Update folder structure
	folderStructureStore.update((fs) => {
		const root = fs[0];

		// find folder
		let folder = root.children.find((f) => f.id === folderName);

		// 3a. If folder doesn't exist → create it
		if (!folder) {
			folder = {
				id: folderName,
				name: folderName,
		...
		// 3b. Add file inside folder
		folder.children.push({
			id: newId,
			name: `zadatak${newIndex}.c`
		});
